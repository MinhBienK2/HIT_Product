   ...
   (messageId, ArrayFriendId) => (dispatch, getState) => {
      ArrayFriendId.forEach(async (ele) => {
         const res = await Axios({
            method: "GET",
            url: `http://localhost:3000/api/v1/notifications/message-id/${messageId}/friend-id/${ele}`,
         ...
         if (res.data) {
            ...
            const response1 = await Axios({
               method: "PATCH",
               url: `http://localhost:3000/api/v1/notifications/unread/message-id/${res.data.messageId}/friend-id/${res.data.friendId}`,
               withCredentials: true,
            });
            ...
            if (response.status === "success") {
               ...
               dispatch(
                  updateCountMessageUnread(response.data.countMessageSended)
               );
               dispatch(setFriendId(ele));
            }
         }
