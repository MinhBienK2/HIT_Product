...
export const fetchChatInput = (valueMessage) => (dispatch, getState) => {
   if (getState().chat.listContent.length === 0) {
      Axios({
         method: "POST",
         url: `http://localhost:3000/api/v1/messages/friend-id/${
         ...
         .then(async (data) => {
            if (data.status === "success") {
               ...
               localStorage.setItem("messageIdOfNotification", data.messageId);
               Axios({
                  method: "POST",
                  url: `http://localhost:3000/api/v1/messages/${data.messageId}`,
                  withCredentials: true,
                  data: {
                  ...
                  .then(async (data1) => {
                     ...
                     const response1 = await Axios({
                        method: "POST",
                        url: `http://localhost:3000/api/v1/notifications/message-id/${
                           data.messageId
                     ...
                     if (response1.status === "success") {
                        await Axios({
                           method: "PATCH",
                           url: `http://localhost:3000/api/v1/notifications/read/message-id/${
                              data.messageId
                           }/friend-id/${JSON.parse(user).id}`,
                           withCredentials: true,
                        });
                     ...
                     getState().chat.arrayFriendId.forEach(async (ele) => {
                        const response = await Axios({
                           method: "POST",
                           url: `http://localhost:3000/api/v1/notifications/message-id/${data.messageId}/friend-id/${ele}`,
                           withCredentials: true,
                        });
      ...
      Axios({
         ...
         .then((data1) => {
            ...
            getState().chat.arrayFriendId.forEach(async (ele) => {
               ...
               Axios({
                  method: "PATCH",
                  url: `http://localhost:3000/api/v1/notifications/unread/message-id/${
                     getState().chat.messageId
...
export const fetchChatView = () => (dispatch, getState) => {
   Axios({
      method: "GET",
      withCredentials: true,
      url: `http://localhost:3000/api/v1/messages/get-message/${
         getState().chat.friendId
      }`,
   })
      .then((data) => {
