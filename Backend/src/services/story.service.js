...
const createStory = CatchAsync(async (req, res, next) => {
    ...
    if (req.file.mimetype.startsWith("video")) {
        linkName = `process.env.APP_NODE_BACKEND_URL/videos/stories/${req.file.filename.replace(
            ".m3u8",
            ""
        )}/${req.file.filename}`;
        streamStory(req.file);
    } else if (req.file.mimetype.startsWith("image")) {
        linkName = `process.env.APP_NODE_BACKEND_URL/images/stories/${req.file.filename}`;
    }
